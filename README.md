# Tariq Chehardy LLC — Absolute Precision

The **official site** of Tariq Chehardy LLC — senior expert-level software: custom code, custom scripts, commissioned programming, reverse engineering, PowerShell & internal production work, and design.

**Live:** https://tariqchehardyllc.github.io/

## Specializations

| | |
|---|---|
| **Custom Code** | production-grade software, built to spec |
| **Custom Scripts** | purpose-built automation and tooling |
| **Commissioned Programming** | client-commissioned engineering, end to end |
| **Reverse Engineering** | deep program-internal analysis |
| **PowerShell & Internal Production** | prototype stage to internal production |
| **Design** | modern, minimal interface and product design |

## Ventures

- **[Poke](poke.html)** — a super-intelligence agent compiled into a single .skill file, shipped as poke-core. Developed in collaboration with Privacy+ Technologies Inc.
- **PrivacyKit** — privacy-first tooling
- **Vantage Analytics** — lightweight analytics for small businesses

## Contact

**Commercial inquiries only. DMs are closed.**

- **Email:** [TariqChehardy@gmail.com](mailto:TariqChehardy@gmail.com)
- **GitHub:** [@tariqchehardyllc](https://github.com/tariqchehardyllc)

**Company record**

- **Business address:** 4502 Gaston Ave, Apt 119, Dallas, TX 75246
- **Live revenue:** ~$85,000 annually · $7,083 monthly, fluctuating with operating costs and infrastructure

---

© 2026 Tariq Chehardy LLC
